...
function setTheme(theme) {
  activeTheme = theme;
  activeSkin = null;
  themeLink.href = 'themes/' + theme + `/theme-${theme}.${themeExt}`;
  themeSelect.value = theme;
  desktop.className = 'desktop grow ' + theme ;
...
async function setSkin(skin) {
  ...
  // fetch('themes/' + activeTheme + '/skins/' + activeSkin + '.css')
  //   .then(response => response.text())
  //   .then(css => {
  //     css = css.replace(/:root/g, `.${activeTheme}`);
  //     skinLink.innerHTML = css;
  //   }); 
  const icss = await readSkin(activeSkin+ '.css');
  skinLink.innerHTML = icss;
}
...
async function readSkin(file_css) {
  ...
  const asyncFn2 = async (match, ...args) => {
    console.log('Importing CSS:', match, 'args:', args  );
    const content = await readSkin(args[0]);
    css = css.replace(match, content); // Replace the @import statement with the imported CSS content
    // skinLink.innerHTML += content; // Append the imported CSS to the skin link
    // debugger
    // const url = args[0];
    // const resolvedUrl = new URL(url, response.url).href;
    // const res = await fetch(resolvedUrl);
    // if (!res.ok) {
    //   console.warn(`Failed to fetch ${resolvedUrl}: ${res.status} ${res.statusText}`);
    //   return match; // Return the original @import statement on failure
    // }
    // let importedCss = await res.text();
    // importedCss = importedCss.replace(/:root|:scope/g, `.${activeTheme}`);
    // skinLink.innerHTML += importedCss; // Append the imported CSS to the skin link
  }


  css = css.replace(/:root|:scope/g, `.${activeTheme}`);
  if(css.indexOf('@import') >= 0) {
    console.warn('Skin CSS still contains :root selectors. This may cause issues with scoping.');
    // css = css.replace(/@import\s+['"]([^'"]*)['"]/g, `.${activeTheme}`);
    css = css.replace(/@import\s+['"]([^'"]+)['"]\s*;/gm,  (match, ...args) => {
      promises.push(asyncFn2(match, ...args));
      return match;
    });
  }
